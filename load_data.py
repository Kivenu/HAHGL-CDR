...
def LoadData_with_bond_features(Drug_info_file, Drug_feature_file, Cell_line_info_file, Genomic_mutation_file,
                                Gene_expression_file, Methylation_file, Cancer_response_exp_file, IC50_threds_file):
    """
    Load drug molecular graph data with bond features
    Returns: drug_feature_with_bonds, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs
    """
    print('Loading data with bond features...')
    reader = csv.reader(open(Drug_info_file, 'r'))
    rows = [item for item in reader]

    print('1-loading drugs information with bond features')
    # [drugid->pubchemid]
    drugid2pubchemid = {item[0]: item[5] for item in rows if item[5].isdigit()}
    ...
    # load drug molecular graph feature with bond features [pubchemid->feature]
    drug_pubchem_id_set = []
    drug_feature_with_bonds = {}
    for each in os.listdir(Drug_feature_file):
        drug_pubchem_id_set.append(each.split('.')[0])
        pkl_file = open('%s/%s' % (Drug_feature_file, each), 'rb')
        # New pkl format: (feat_mat, adj_list, degree_list, bond_matrix)
        feat_mat, adj_list, degree_list, bond_matrix = pickle.load(pkl_file)
        drug_feature_with_bonds[each.split('.')[0]] = [feat_mat, adj_list, degree_list, bond_matrix]
    assert len(drug_pubchem_id_set) == len(drug_feature_with_bonds.values())
    ...
    nb_drugs = len(set([item[1] for item in data_new]))
    print('All %d pairs across %d cell lines and %d drugs.'%(len(data_new),nb_celllines,nb_drugs))
    return drug_feature_with_bonds, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs
...
def LoadData_gdsc(Drug_info_file, Drug_feature_file, Cell_line_info_file, Genomic_mutation_file, 
                     Gene_expression_file, Methylation_file, Cancer_response_exp_file, IC50_threds_file):
    """
    GDSC drug data loading function
    Returns: drug_feature, mutation_feature, gexpr_feature, methylation_feature, data_new, nb_celllines, nb_drugs
    """
    print('Loading data (gdsc format)...')
    reader = csv.reader(open(Drug_info_file, 'r'))
    rows = [item for item in reader]

    print('1-loading drugs information (converting bond features to basic features)')
    # [drugid->pubchemid]
    drugid2pubchemid = {item[0]: item[5] for item in rows if item[5].isdigit()}
    ...
    print('3-loading response information')
    # load cancer response information
    cancer_response_exp = pd.read_csv(Cancer_response_exp_file, sep=',', header=0, index_col=[0])
    
    # Filter rows with drug information
    drug_match_list = [item for item in cancer_response_exp.index if item.split(':')[1] in drugid2pubchemid.keys()]
    cancer_response_exp_filtered = cancer_response_exp.loc[drug_match_list]
    
    data_new = []
    for each_drug in cancer_response_exp_filtered.index:
        for each_cellline in cancer_response_exp_filtered.columns:
            # Extract numeric part of drug ID
            drug_id = each_drug.split(':')[-1]
            if drug_id in drugid2pubchemid:
                pubchemid = drugid2pubchemid[drug_id]
                # Check if both drug and cell line have feature information
                if str(pubchemid) in drug_pubchem_id_set and each_cellline in gexpr_feature.index:
                    if not pd.isna(cancer_response_exp_filtered.loc[each_drug, each_cellline]):
                        ic50 = float(cancer_response_exp_filtered.loc[each_drug, each_cellline])
                        if pubchemid in drug2thred:
                            thred = drug2thred[pubchemid]
                            if ic50 <= thred:
                                data_new.append([each_cellline, pubchemid, 1])  # Sensitive
                            else:
                                data_new.append([each_cellline, pubchemid, -1])  # Resistant
