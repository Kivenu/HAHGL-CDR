...
from load_data import LoadData_ccle
...
parser = argparse.ArgumentParser(description="HAHGL_CDR CCLE - Run specific Repeat and Fold")
...
parser.add_argument('--threshold', type=float, default=0.6, help='Adaptive hypergraph sparsification parameter')
...
parser.add_argument('--target_fold', type=int, default=2, help='Specify the fold to run (starting from 0)')
parser.add_argument('--outfile', type=str, default="HAHGL_CDR_CCLE_results", help='Output filename prefix')
...
for split_idx, split_data in enumerate(all_splits):
    ...
    print(f"Number of positive samples: {label_pos.sum().item()}")
    print(f"Number of negative samples: {(label_pos == 0).sum().item()}")
    print(f"Label range: [{label_pos.min().item():.4f}, {label_pos.max().item():.4f}]")

    print(f"CCLE - HAHGL_CDR Configuration:")
    ...
    print(f"Best result - Seed {args.seed}, Repeat {repeat+1}, Fold {fold+1}: {best_result}")
